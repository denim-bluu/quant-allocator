...
> **the lead reviewer plan review (2026-07-07): APPROVED.** Composition-only confirmed; the shared-lint migration stays a deferred docket item.
...
**Lint-placement ruling (BINDING, from the spec's gate review + dispatch):** the generic invariants INV-1 (no bare point) and INV-2 (provenance) would ideally extend the shared gallery lint (`build.py:_lint_outputs`), but that is a prohibited seam. Therefore **ALL pack invariants (INV-1 through INV-4 + numeric faithfulness + gate-respect) live in E2's own builder module `flagships/packs/lints.py` for this plan.** The eventual migration of the generic checks into the shared gallery lint is deferred to the integration task and recorded in the docket. The shared `build.py:_lint_outputs` is not modified.
...
- **Model policy:** senior track implementer (this is statistics-adjacent composition/lint code), senior track reviewer, with a **batched the lead reviewer numerics + copy gate at the END of wave-2 batch-1** (the `HELD FOR THE NUMERICS GATE` header on `e2_pack.json` holds it until then).
...
git commit -m "feat(e2): demo generator + committed pack JSON (held for gate)"
```
...
**Authored here (this plan's the lead reviewer numerics + copy gate audits these):**
