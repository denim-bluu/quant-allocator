...
> **the lead reviewer plan review (2026-07-07): APPROVED. Ruling P3-D2 RATIFIED** — the PowerGate compares EFFECTIVE N (design-effect deflated), the only reading reconciling the spec's 12-event line with the mandated refusal at ~15 correlated events. The gate audits the deflator at certification.
...
- **Model policy.** senior track implementer (statistics-bearing card), senior track reviewer, with a single batched **the lead reviewer numerics + copy gate at the END of the wave-2 batch-1** (the committed `p3_hirefire.json` carries the HELD-FOR-GATE header and is not published — cards.yaml stays `planned` — until that gate certifies the numbers and the prior-scale citation).
...
This flip is gated on the batched **the lead reviewer numerics + copy certification** of the committed `site/data/p3_hirefire.json` (P3-D1 citation and P3-D2 ruling in particular).
