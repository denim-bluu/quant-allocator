...
**Scope:** The idea-gallery static site (shell + index), demo pages and full
method specs for the selected five cards (S1, S2, X1, X2, E1), demo-data
generators, and publication to a new public GitHub repo with GitHub Pages.
...
src/quant_allocator/
  demo_data/
    ...
    s1_ledger.py  s2_tearsheet.py  x1_atlas.py  x2_playground.py
  site/
    __init__.py
...
Wave 1: S1, S2, X1, X2, E1 are `live`; the other 15 are `planned` (no
...
## 9. Method specs — the five wave-1 documents
...
content itself. Math in LaTeX; figures, where needed, as inline SVG
...
1. **Specs first (the lead reviewer):** S1, X1 → S2, X2, E1 (the math the demos render
   is pinned before any rendering exists).
