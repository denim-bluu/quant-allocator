...
- Tests (offline, in the existing suite): JSON schema validity; byte-for-byte
  ...
  threshold.
- Every generator lands via SDD with a **numerics review** on the
...
list). Depth: S1 and X1 full (lead-reviewer-authored end to end); S2 and X2 full
...
- Visual QA gate before Pages: Playwright screenshots of every page (light +
  dark, desktop + one mobile width), reviewed at senior level against the
  demo-layer spec.
